...
#    if lag_x == 0:
#        analogs_CV = np.reshape(LLR.data.ana,(dimx,dimD*lenC)); successors_CV = np.reshape(LLR.data.suc,(dimx,dimD*lenC));
#        analogs_CV_prev = np.reshape(LLR.data_prev.ana,(dimx,dimD*lenC)); successors_CV_prev = np.reshape(LLR.data_prev.suc,(dimx,dimD*lenC));
#    else:
#        if (tx < lag_x):
#           indCV = np.where( np.abs((tx-LLR.data.time) % LLR.time_period) <= lag_Dx & np.abs(tx-LLR.data.time)  >= lag_x, dtype =int)                #np.arange(lag_x,TC-1);
#        elif (tx > TC-lag_x):
#            indCV =  np.where( np.abs((tx-LLR.data.time) % LLR.time_period) <= lag_Dx & np.abs(tx-LLR.data.time)  <= lag_Dx-lag_x, dtype =int)       #np.arange(0,TC-lag_x);
#        else:
#            indCV =  np.where( np.abs((tx-LLR.data.time) % LLR.time_period) <= lag_Dx & np.abs(tx-LLR.data.time) > lag_x, dtype =int)       #np.concatenate((np.arange(0,tx-lag_x+1), np.arange(tx +lag_x,TC-1)))
#        analogs_CV = np.reshape(LLR.data.ana[...,indCV],(dimx,len(indCV)*dimD));
#        successors_CV = np.reshape(LLR.data.suc[...,indCV],(dimx,len(indCV)*dimD));
#        analogs_CV_prev = np.reshape(LLR.data_prev.ana[...,indCV],(dimx, len(indCV)*dimD ));
#        successors_CV_prev =np.reshape(LLR.data_prev.suc[...,indCV],(dimx, len(indCV)*dimD ));    
