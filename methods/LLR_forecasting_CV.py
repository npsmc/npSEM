...
""" AnDA_analog_forecasting.py: Apply the analog method on data of historical 
...
def m_LLR(x, tx, ind_x, LLR):
    ...
    if len(LLR.data.ana.shape) == 1:
        ...
        dimD = 1
        dimx, _ = LLR.data.ana.shape
    else:
        dimx, dimD, _ = LLR.data.ana.shape
    try:
        indCV = (np.abs((tx - LLR.data.time)) % LLR.time_period <= lag_Dx) \
                & (np.abs(tx - LLR.data.time) >= lag_x)
    except:
    ...
    if LLR.gam != 1:
        ...
        analogs_CV_prev = np.reshape(LLR.data_prev.ana[..., indCV_prev],
                                     (dimx_prev, lenD_prev * dimD_prev))
        successors_CV_prev = np.reshape(LLR.data_prev.suc[..., indCV_prev],
                                        (dimx_prev, lenD_prev * dimD_prev))
