...
def sampling_discrete(W, m):
    """
        Discrete sampling, ADDED by TRANG ###

    :param W:
    :param m:
    :return:  m indices given N weights
    """
    cumprob = np.cumsum(W)
    n = W.size
    R = np.random.rand(m)
...
def resampling_sys(W):
    """
        systematic resampling with respect to multinomial
        distribution, ADDED by TRANG

    :param W:
    :return:  N-set of indices given N weights
    """
    N = np.size(W)
    u0 = np.random.rand(1)
    u = (range(N) + u0).T / N
